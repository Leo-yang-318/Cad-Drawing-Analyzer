...
def refine_box_by_pixels(img_no_blue, poly, padding=2):
    ...
    if coords is not None:
        ...
        return np.array(
            [[rx + x_start, ry + y_start], [rx + rw + x_start, ry + y_start], [rx + rw + x_start, ry + rh + y_start],
             [rx + x_start, ry + rh + y_start]]).astype(np.float32)
    return poly.astype(np.float32)


def merge_boxes_full(poly_list):
    if not poly_list: return np.array([])
